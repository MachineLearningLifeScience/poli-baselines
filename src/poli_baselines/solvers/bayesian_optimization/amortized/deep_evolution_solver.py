...
from poli_baselines.solvers.bayesian_optimization.amortized import base_solver
from poli_baselines.solvers.bayesian_optimization.amortized import data
...
def apply_mutations(
    samples, mutation_types, pos_masks, mutations, use_assignment_mutations=False
):
...
def sample_log_probs_top_k(log_probs, rng, temperature=1.0, k=1):
...
class JaxMutationPredictor(object):
    ...
    def __init__(
        ...
        self.make_state, self._opt_update_state, self._get_params = adam(learning_rate)
...
def run_model_and_compute_reinforce_loss(
    params, run_model_fn, rewards, inputs, actions, n_mutations, entropy_weight=0.1
):
...
class MutationPredictorSolver(base_solver.BaseSolver):
    ...
    def __init__(self, domain, model_fn=build_model_stax, random_state=0, **kwargs):
        """Constructs solver.
        ...
        mut_loss_fn = functools.partial(
            run_model_and_compute_reinforce_loss, n_mutations=len(self.mutations)
        )
    ...
    def _get_best_samples_from_last_batch(
        self, population, n=1, discard_duplicates=True
    ):
    ...
    def _select(self, population):
        if self.cfg.use_selection_of_best:
            ...
            idx = np.random.choice(
                len(structures), self.batch_size, replace=True
            ...
            if (
                len(last_batch) > self.batch_size
            ):  # pytype: disable=attribute-error  # trace-all-classes
                # Subsample the data
                idx = np.random.choice(
                    len(last_batch), self.batch_size, replace=False
                )  # pytype: disable=attribute-error  # trace-all-classes
    ...
    def propose(
        self, num_samples, population=None, pending_samples=None
    ...
    def _perturb(self, parents, mode="train"):
        ...
        mutation_logprobs = gather_positions(positions, all_mut_log_probs)
        assert mutation_logprobs.shape == (
            output.shape[0],
            self.cfg.num_mutations,
            output.shape[-1] - 1,
        )
    ...
    def _update_params(self, population):
        ...
        self._mut_predictor.update_step(
            rewards=advantage, inputs=all_states[0], actions=dat["actions"]
        )
