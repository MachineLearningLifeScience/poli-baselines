...
from poli_baselines.solvers.bayesian_optimization.amortized import domains
...
def serialize_structures(structures, **kwargs):
    ...
    return [serialize_structure(structure, **kwargs) for structure in structures]
...
def deserialize_structure(serialized_structure, dtype=np.int32):
    ...
    """
    return np.asarray(
        [token for token in serialized_structure.split(domains.SEP_TOKEN)], dtype=dtype
    )
...
def deserialize_structures(structures, **kwargs):
    ...
    """
    return [deserialize_structure(structure, **kwargs) for structure in structures]
...
def population_frame_to_csv(
    frame, path_or_buf=None, domain=None, index=False, **kwargs
):
...
class Sample(object):
    ...
    structure: np.ndarray = attr.ib(
        factory=np.array, converter=_to_immutable_array
    )  # pytype: disable=wrong-arg-types  # attr-stubs
    reward: float = attr.ib(converter=_NumericConverter(float), default=None)
    batch_index: int = attr.ib(
        converter=_NumericConverter(int, min_value=0), default=None
    )
    infeasible: bool = attr.ib(
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and np.array_equal(self.structure, other.structure)
        )
    ...
    def to_tfexample(self):
        ...
        features = dict(
            ...
            reward=tf.train.Feature(float_list=tf.train.FloatList(value=[self.reward])),
...
def samples_from_arrays(structures, rewards=None, batch_index=None, metadata=None):
    ...
    if len(metadata) != len(rewards):
        raise ValueError(
            "Metadata and rewards must be same length. Are %s and %s"
    ...
    for structure, reward, batch_index, meta in zip(
        structures, rewards, batch_index, metadata
    ):
        yield Sample(
            structure=structure, reward=reward, batch_index=batch_index, metadata=meta
        )
...
class Population(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, Population):
            raise ValueError(
                "Cannot compare equality with an object of "
                "type %s" % (str(type(other)))
            )

        return len(self) == len(other) and all(s1 == s2 for s1, s2 in zip(self, other))
    ...
    def add_batch(self, structures, rewards=None, batch_index=None, metadata=None):
    ...
    def from_csv(cls, path):
        ...
        return cls.from_frame(
            population_frame_from_csv(path).drop(
