...
def get_random_state(seed_or_state):
    ...
    if isinstance(seed_or_state, int):
        ...
        raise ValueError(
            "Numpy RandomState or integer seed expected! Got: %s" % seed_or_state
        )
    else:
...
def tensors_from_dataset(dataset):
    ...
    shapes_dtypes = zip(
        tf.nest.flatten(dataset.output_shapes), tf.nest.flatten(dataset.output_types)
    )
    tensors = [
...
def hash_structure(structure):
    ...
    raise ValueError(
        "%s can not be safely cast to np.int32 or " "np.float32" % str(structure)
    )
...
def deduplicate_samples(samples, select_best=False):
    ...
    def _sort(to_sort):
        return (
            sorted(to_sort, key=lambda sample: sample.reward, reverse=True)
            if select_best
            else to_sort
        )
...
def group_samples(samples, **kwargs):
    ...
    return group_by_hash(
        samples, hash_fn=lambda sample: hash_structure(sample.structure), **kwargs
    )
...
def get_instance(instance_or_cls, **kwargs):
    ...
    """
    if (
        inspect.isclass(instance_or_cls)
        or inspect.isfunction(instance_or_cls)
        or isinstance(instance_or_cls, functools.partial)
    ):
...
def pd_option_context(
    width=999, max_colwidth=999, max_rows=200, float_format="{:.3g}", **kwargs
):
...
def get_indices(
    valid_indices, selection, map_negative=True, validate=False, exclude=False
):
